...
Prior to the development of this tool, identifying size imbalances required manual review of inventory positions across multiple styles and locations. The automated process transformed this into an exception-based workflow, allowing attention to be focused only on styles requiring investigation.
...
The process imports size-level inventory data and calculates the percentage contribution of each size to the total stock position. The model automatically highlights styles where a single size exceeds a predefined threshold, enabling rapid identification of inventory anomalies.
...
The screenshots below demonstrate the workflow used to identify size-level inventory imbalances across the retail network. They show how stock data was extracted, transformed, analysed, and automatically flagged to highlight styles with unusual size distributions that may require further investigation.
...
![Image Alt](https://github.com/pembers1/project-1/blob/main/Screenshot%202026-07-08%20112041.png?raw=true)
